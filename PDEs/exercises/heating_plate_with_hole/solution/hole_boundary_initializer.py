...
def set_hole_boundary_conditions(nodes_matrix):
    ...
    # setting hole temperature to the room temperature:
    set_hole_temperature(nodes_matrix, room_temperature)


def set_hole_temperature(nodes_matrix, temperature):
    for i in range(rows):
        for j in range(columns):
            if geo.is_inside_hole(i, j):
                nodes_matrix[i, j] = temperature
